  ...
  <transition appear name="slide">
    ...
    >
      <a
        :href="link"
        :key="link"
        :title="isFacebook(link) && 'Facebook' || isInstagram(link) && 'Instagram' || null"
        target="_blank"
        v-for="link in socialLinks"
      >
        <svg width="14" height="26" viewBox="0 0 15 28" xmlns="http://www.w3.org/2000/svg" v-if="isFacebook(link)">
          <path
        ...
        <svg width="26" height="26" viewBox="7 7 26 26" xmlns="http://www.w3.org/2000/svg" v-else-if="isInstagram(link)">
...
export default {
  ...
  setup() {
    ...
    /** @param link {string} */
    const isFacebook = link => link.includes('facebook.com') || link.includes('fb.com')
    /** @param link {string} */
